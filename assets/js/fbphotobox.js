(function ($) {
  ...
  FBPhotoBox.prototype = {
    init: function () {
      ...
      this.lazyLoadImage.onload = function () {
      ...
      $(".fbphotobox-container-left").hover(
        function () {
          $(".fbphotobox-image-stage-overlay").fadeIn(
            $this.settings.imageOverlayFadeSpeed
          );
        },
        function () {
          $(".fbphotobox-image-stage-overlay").fadeOut(
            $this.settings.imageOverlayFadeSpeed
          );
        }
      );
      ...
      // Setup click trigger
      $("." + this.settings.containerClassName).on(
        "click",
        "." + this.settings.imageClassName,
        function () {
          $this.show($(this));
        }
      );
      ...
      this.leftArrow.click(function () {
        var image = $(
          "." +
            $this.settings.containerClassName +
            " ." +
            $this.settings.imageClassName
        ).get($this.leftArrow.attr("data-prev-index"));
      ...
      this.rightArrow.click(function () {
        var image = $(
          "." +
            ...
            " ." +
            $this.settings.imageClassName
        ).get($this.rightArrow.attr("data-next-index"));
        if (image) $this.show($(image));
      });
    ...
    show: function (image) {
      ...
      this.fbpMainImage.attr("content-type", image.attr("author"));
      this.fbpMainImage.attr("author", image.attr("author"));
      this.fbpMainImage.attr("data-des", image.attr("data-des"));
      this.fbpMainImage.attr("post-id", image.attr("post_id"));
      this.fbpMainImage.attr("author-img-src", image.attr("author-img-src"));
    ...
    hide: function () {
      ...
      var from = {
        posX: this.fbpMainImage.offset().left,
        posY: this.fbpMainImage.offset().top,
        widthSrc: this.fbpMainImage.width(),
        heightSrc: this.fbpMainImage.height(),
      };
      var to = {
        posX: this.fbpMainImage.posX,
        posY: this.fbpMainImage.posY,
        widthSrc: this.fbpMainImage.widthSrc,
        heightSrc: this.fbpMainImage.heightSrc,
      };
      var tmpNode = $('<img src=""/>').attr(
        "src",
        this.fbpMainImage.attr("src")
      );
      this.fbpMainContainer.fadeOut({
        easing: "easeOutQuint",
        queue: false,
        duration: 200,
      });
    ...
    initDOM: function () {
      var html = [
        ...
        '<img src="./assets/img/fbphotobox/close.png" style="height:10px;width:10px"/>',
        "</a>",
        ...
        '<a class="fbphotobox-fc-close-btn" href=""><i class="fas fa-times"></i></a>',
        '<div style="clear:both"></div>',
        "</div>",
    ...
    initSettings: function () {
      ...
      if (this.settings.leftBgColor != "") {
        $(".fbphotobox-container-left").css(
          "backgroundColor",
          this.settings.leftBgColor
        );
      }

      if (this.settings.rightBgColor != "") {
        $(".fbphotobox-container-right").css(
          "backgroundColor",
          this.settings.rightBgColor
        );
      }

      if (this.settings.footerBgColor != "") {
        $(".fbphotobox-container-left-footer-bg").css(
          "backgroundColor",
      ...
      if (this.settings.overlayBgColor != "") {
        $(".fbphotobox-overlay").css(
          "backgroundColor",
          this.settings.overlayBgColor
        );
      }
    ...
    displayScroll: function () {
      $("body").css({
        // width: this.bodyDimension.width,
        // height: this.bodyDimension.height,
        overflow: "auto",
      });
    },
    refreshBoxSize: function (image) {
      ...
      var maxWidth = Math.max(
        $(window).width() -
          this.settings.rightWidth -
          this.settings.normalModeMargin * 2,
        this.settings.minLeftWidth
      );
      var maxHeight = Math.max(
        $(window).height() - this.settings.normalModeMargin * 2,
      ...
      $(".fbphotobox-image-content").css(
        "height",
        leftContainer.height() - $(".fbphotobox-close-btn").height()
      );
    ...
    refreshFullScreenSize: function () {
      $(".fbphotobox-fc-main-image").css({
        "max-width":
          $(window).width() -
          $(".fc-left-arrow").width() -
          $(".fc-right-arrow").width() -
          20,
        "max-height":
          $(window).height() -
          $(".fbphotobox-fc-header").outerHeight(true) -
          $(".fbphotobox-fc-footer").outerHeight(true),
      });
      $(".fbphotobox-fc-main-image")
        .closest("div")
        .css(
          "height",
          $(window).height() -
    ...
    addTags: function (tagsCo) {
      ...
      var tagNode = $(document.createElement("div")).attr(
        "class",
        "fbphotobox-tag"
      );
